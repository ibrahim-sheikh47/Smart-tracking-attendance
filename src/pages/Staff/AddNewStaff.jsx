...
const AddNewStaff = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const db = getFirestore();
      await setDoc(doc(db, "employees", result.data.uid), employeeWithId);

      console.log("Employee created successfully:", result.data.uid);
  ...
  // Generate a simple employee ID (You can adjust this based on your needs)
    ...
    <div className="p-10">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <CustomButton
            title={isSubmitting ? "Processing..." : "Create New Employee"}
            style={"w-[200px] text-white h-10"}
            onClick={handleSubmit(onSubmit)}
